...
DBMS = ["sqlite", "mysql", "mariadb", "postgresql", "percona"]
...
def run(database, input_dir, output_dir=None, config_file=None, fuzzer=None):
  ...
  if database == "sqlite":
    cmd = f"{fuzzer} -i {input_dir} -o {output_dir} -M {output_id} -V 45000 -- /home/ossfuzz @@"
  else:
    cmd = f"{fuzzer} -i {input_dir} -o {output_dir} -M {output_id} -t 60000 -V 45000 -- {ROOTPATH}/build/db_driver"
